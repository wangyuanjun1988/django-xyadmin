layui.use(['form','layer','element','upload','laydate'],function(){
    ...
    var uploadInst = upload.render({
        elem: '#btn_upload_img'
        ,url: '/xyadmin/page_management/upload_readernotice_cover_img'
        ,accept:'images'
        ,size: 512
    ...
    form.on("submit(volunteerstation_add)",function(data){
        //弹出loading
        var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        // 实际使用时的提交信息
         $.post("readernotice_add",data.field,function(res){
             if (res.code == '1') {
                setTimeout(function(){
