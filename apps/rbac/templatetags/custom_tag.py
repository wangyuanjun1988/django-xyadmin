...
@register.simple_tag
def rbac_head_menu(request):
    head_menu = request.session[settings.SESSION_HEAD_MENU_KEY]

    option_str = '<li class="layui-nav-item " data-menu="{menu_code}"><a href="javascript:;"><i class="{i_class_name}" data-icon="{data_icon}">{data_icon}</i><cite>{title}</cite></a></li>'

    head_menu_html = ''

    for item in head_menu:
        if item.get('title'):
            head_menu_html += option_str.format(menu_code=item['menu_code'],
                                                i_class_name=item['i_class_name'],
                                                data_icon=item['data_icon'],
                                                title=item['title'])
    return head_menu_html.replace("'", "")
...
@register.filter
def rbac_menu_but(request,menu_code):
    permission_url_list = request.session[settings.SESSION_MENU_KEY][settings.PERMISSION_MENU_KEY]
    for per in permission_url_list:
        if menu_code == per['menu_code']:
            return 'true'
    return 'false'
